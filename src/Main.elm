...
sampleTable a b a2 b2 = [blockquote [] [text "This alchemy is valid - ",table [] [thead [] [th [] [], th [style "border-bottom" "5px solid #111111"] [text <| String.fromChar '💨'], th [style "border-bottom" "5px solid #111111"] [text <| String.fromChar '🌊']]
                                                        ,tr [] [ th [style "border-right" "5px solid #111111"] [text <| String.fromChar '💨'], td [] [text <| String.fromChar '💨'], td [] [text <| String.fromChar '🌊']]
                                                        ,tr [] [ th [style "border-right" "5px solid #111111"] [text <| String.fromChar '🌊'], td [] [text <| String.fromChar a], td [] [text <| String.fromChar b]]]],
                                blockquote [] [text "This alchemy is not - ",table [] [thead [] [th [] [], th [style "border-bottom" "5px solid #111111"] [text <| String.fromChar '💨'], th [style "border-bottom" "5px solid #111111"] [text <| String.fromChar '🌊']]
                                                        ,tr [] [ th [style "border-right" "5px solid #111111"] [text <| String.fromChar '💨'], td [] [text <| String.fromChar '💨'], td [] [text <| String.fromChar '🌊']]
                                                        ,tr [] [ th [style "border-right" "5px solid #111111"] [text <| String.fromChar '🌊'], td [] [text <| String.fromChar a2], td [] [text <| String.fromChar b2]]]]
                             ]
view : Model -> Html Msg
view model = let pages = (let nextButtonEnable cond = button [onClick NextPage,
                  ...
                  in [
                        ...
                        div [] [h2 [] [text "Using the Table"],
                                ...
                                nextButtonEnable <| length model.alchemySet == 3 && notMember '_' (model.alchemySet ++ List.concat model.combinationTable)],
                        ...
                        div [] ([h2 [] [text "Alchemical Rules - Identity"]
                                ,p [] [text """The next rule is the rule of identity. Our alchemy needs to have an identity element, which
                                            ...
                                            element it is combined with to change. For example:""" ]] ++ sampleTable '🌊' '🌊' '💨' '🌊' ++
                        ...
                        div [] ([h2 [] [text "Alchemical Rules - Commutativity"],
                                p [] [text """The rule of commutativity says that the order you combine elements in shouldn't matter.
                                            This means that our table should be symmetric along the diagonal. For example:"""]] ++ sampleTable '🌊' '💨' '💨' '💨' ++[p [] [ text """To move on, make the 3 by 3 table have symmetry across the diagonal."""],
                                nextButtonEnable <| (&&) (length model.alchemySet == 3 && notMember '_' model.alchemySet) <| model.combinationTable == transpose model.combinationTable]),
                        div [] [h2 [] [text "All Alchemical Rules"],
                               ...
                               nextButtonEnable <| (&&) ((&&) (length model.alchemySet == 3 && notMember '_' model.alchemySet && Just model.alchemySet == head model.combinationTable && Just model.alchemySet == head (transpose model.combinationTable)) <| foldl (&&) True <| List.map (\c -> member c model.alchemySet) <| List.concat model.combinationTable) <| foldl (&&) True <| List.map (\row -> row == unique row) <| append model.combinationTable <| transpose model.combinationTable
                               ],
  ...
  div []
    ...
    , table [] [thead [] <| concat [[th [] []],
                       List.map (\(c, i) -> th [onClick <| ChangeSet i,style "border-bottom" "5px solid #111111"] [text <| String.fromChar c]) <| zip model.alchemySet alchemyRange,
                       [th [] [button [onClick PopElement] [text "-"], button [onClick PushElement] [text "+"]]]
                       ],
                tbody [] <| List.map
                    (\(alchemyElement,row,n) -> tr [] <| append ([th [onClick <| ChangeSet n, style "border-right" "5px solid #111111"] [text <| String.fromChar alchemyElement]]) <| List.map
    ...
    , div [style "visibility" <| if model.elementSelectDisplay /= Hidden then "visible" else "collapse"] [text "Select an Element Below:", table [] [
